...
def get_params():
    ...
    parser.add_argument("--train_name", default='ObserveBranch', type=str)
    parser.add_argument("--do_plot", default=True)
    args, unknown = parser.parse_known_args()
    return args


class ObserveTransformer(BertPreTrainedModel):

    def __init__(self, config):
        super(ObserveTransformer, self).__init__(config)
...
def pretrain_observe_branch(args):
    ...
    if not os.path.isfile(os.path.join(modeldir, 'pytorch_model.bin')):
        ...
        # Prepare model
        model = ObserveTransformer.from_pretrained(args.bert_model, cache_dir=os.path.join(PYTORCH_PRETRAINED_BERT_CACHE, 'distributed_{}'.format(-1)))
        model.to(args.device)
        if n_gpu > 1:
            model = torch.nn.DataParallel(model)

        # Load training data
        trainDataObject = ObserveBranchData(args, split='train', tokenizer=tokenizer)
        valDataObject = ObserveBranchData(args, split='val', tokenizer=tokenizer)

        # Start training
        logger.info('*** Observe branch training ***')
    ...
    model = ObserveTransformer.from_pretrained(modeldir)
    model.to(args.device)
    if n_gpu > 1:
        model = torch.nn.DataParallel(model)

    # Get Observe Branch embeddings for each dataset split
    logger.info('*** Get observe branch embeddgins for each data split ***')
    trainDataObject = ObserveBranchData(args, split='train', tokenizer=tokenizer)
    valDataObject = ObserveBranchData(args, split='val', tokenizer=tokenizer)
    testDataObject = ObserveBranchData(args, split='test', tokenizer=tokenizer)
    branch_embeddings(args, model, outdatadir, trainDataObject, split='train', branch_name='observe')
    branch_embeddings(args, model, outdatadir, valDataObject, split='val', branch_name='observe')
    branch_embeddings(args, model, outdatadir, testDataObject, split='test', branch_name='observe')
    logger.info('*** Pretraining observe branch done!')
