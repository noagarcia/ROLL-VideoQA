...
def get_params():
    ...
    parser.add_argument("--train_name", default='ReadBranch', type=str)
    parser.add_argument("--do_plot", default=True)
    args, unknown = parser.parse_known_args()
    return args


class ReadTransformer(BertPreTrainedModel):

    def __init__(self, config):
        super(ReadTransformer, self).__init__(config)
...
def pretrain_read_branch(args):
    ...
    if not os.path.isfile(os.path.join(modeldir, 'pytorch_model.bin')):
        ...
        # Prepare model
        model = ReadTransformer.from_pretrained(args.bert_model, cache_dir=os.path.join(PYTORCH_PRETRAINED_BERT_CACHE, 'distributed_{}'.format(-1)))
        model.to(args.device)
        if n_gpu > 1:
            model = torch.nn.DataParallel(model)

        # Load training data
        trainDataObject = ReadBranchData(args, split='train', tokenizer=tokenizer)
        valDataObject = ReadBranchData(args, split='val', tokenizer=tokenizer)
    ...
    trainDataObject = ReadBranchData(args, split='train', tokenizer=tokenizer)
    valDataObject = ReadBranchData(args, split='val', tokenizer=tokenizer)
    testDataObject = ReadBranchData(args, split='test', tokenizer=tokenizer)
